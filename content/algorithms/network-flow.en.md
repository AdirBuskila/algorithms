...
- **(d) The −1 vs −arbitrary asymmetry.** *(2026 גרסה 2, Q3 section א)* — with integer capacities: if lowering an edge's capacity **by exactly 1** lowers the max flow, that edge lies in **some min cut**; and an edge in **no** min cut tolerates a −1 with zero effect (it has $\ge 1$ slack against every cut). **Both statements break for arbitrary decreases** — a large enough drop creates a *new* bottleneck even on an edge that was in no min cut, so "any decrease that lowers the flow proves min-cut membership" is **false**.
