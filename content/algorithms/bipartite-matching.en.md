...
- **The numbers $\alpha, \eta, \mu$ and covers.** *(2026 גרסה 2, Q3 section ה)* — **every** matching is $\le$ **every** vertex cover ($\mu \le \eta$ is the pointwise version); equality is König and holds **only** in bipartite graphs — the triangle $C_3$ has $\mu = 1$ but $\eta = 2$. The endpoints of a **maximal** matching always form a vertex cover (that's the 2-approximation engine — not necessarily minimum); but the endpoints of a max**imum** matching are **not** a "maximal" cover (isolated vertices break it). Gallai ties it together: $\alpha(G) + \eta(G) = \lvert V\rvert$.
